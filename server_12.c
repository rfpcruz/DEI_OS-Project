...
Estimativa de horas de trabalho de cada aluno:
...
/*
gcc server_12.c conf.c -lpthread -D_REENTRANT -Wall -DDEBUG=1 -o v12
*/
#include <stdio.h>
#include <sys/types.h>
...
int shmid1, shmid2; //ID da memoria partilhada

Stati*  shared_mem; // ponteiro para a memória partilhada
ESTATISTICA_FINAL* shared_mem_estatisticas;// ponteiro para a memória partilhada
...
void handler(int sinal) {
	#if DEBUG
		printf("\nRECEBI UM SINAL  ************* ID: %d\n", sinal );
	#endif

	if (sinal == SIGINT)
	{
	  printf ("Sinal SIGINT. A terminar ...\n");
	  ...
	  shmdt(shared_mem_estatisticas); // faz o detach da memória partilhada 2
	  shmctl(shmid1,IPC_RMID,0); // remove a memória partilhada 1
	  shmctl(shmid2,IPC_RMID,0); // remove a memória partilhada 2
...
int init() {
	...
	printf("Configurações iniciais do servidor:\n");
	printf("Porto para o servidor:     %d\n", port);
	printf("Politica de escalonamento: %d (0 - FIFO; 1 - HTML; 2 - GZ)\n", policy);
	...
	//criação da memória partilhada para a estatistica
   	if( (shmid2 = shmget(IPC_PRIVATE, sizeof(ESTATISTICA_FINAL), IPC_CREAT|0700)) < 0) {
       perror("Erro em shmget com IPC_CREAT 2\n");
       exit(1);
   	}
	if((shared_mem_estatisticas = (ESTATISTICA_FINAL*) shmat(shmid2, NULL, 0)) == (ESTATISTICA_FINAL*)-1) {
		perror("Erro em shmat 2");
		exit(1);
	}
	...
	sem_unlink("ESTATISTICA_FINAL_MUTEX");
	estatistica_final_MUTEX = sem_open("ESTATISTICA_FINAL_MUTEX", O_CREAT|O_EXCL, 0700, 1);
...
//Imprime estatisticas acumuladas no ecrã
void handler_u1(int sign) {
	
	sem_wait(estatistica_final_MUTEX);
	printf("Numero total pedidos servidos  html: %d\n",shared_mem_estatisticas[0].conta_0);
	printf("Numero total pedidos servidos    gz: %d\n",shared_mem_estatisticas[0].conta_1);
	printf("Tempo medio para servir pedido html: %f\n",shared_mem_estatisticas[0].media_0);
	printf("Tempo medio para servir pedido   gz: %f\n",shared_mem_estatisticas[0].media_1);
	sem_wait(estatistica_final_MUTEX);

	#if DEBUG
    	printf("Recebi um sinal do tipo SIGUSR1");
	#endif
}

//Reset da estatisticas globais
void handler_u2(int sign) {
	
	sem_wait(estatistica_final_MUTEX);
	shared_mem_estatisticas[0].conta_0 = 0;
	shared_mem_estatisticas[0].conta_1 = 0;
	shared_mem_estatisticas[0].total_pedidos_0 = 0;//isto tem unidade de tempo!
	shared_mem_estatisticas[0].total_pedidos_1 = 0;//isto tem unidade de tempo!
	sem_wait(estatistica_final_MUTEX);
	
	#if DEBUG
  		printf("Recebi um sina do tipo SIGUSR2");
	#endif
...
int statistics_manager(long id) {
    ...
    printf("╚══════════════════════════════════════════════════════════════════════════════╝\n");
    printf("Processo Filho de PID %ld criado, cujo pai é de PID %ld.\n", (long) id, (long) getppid());
	
	signal(SIGUSR1, handler_u1);
	signal(SIGUSR2, handler_u2);

    //MMF
	int fd;
	int conta_global;
	Stati *file; // ponteiro para o MMAP
	struct stat statbuf;
	
	fd = open("server.log", O_WRONLY | O_CREAT);
    
    #if DEBUG
    	printf("CRIEI O FILE ");
    #endif
    /*
    lseek(fd, sizeof( sizeof(Stati)) * 100 , SEEK_SET);
    write(fd,"", 1);
	*/
    file = mmap(NULL, sizeof(Stati)*100, PROT_READ, MAP_PRIVATE, fd, 0);
    ...
    while(1) {
		...
		total_ped_0 = shared_mem_estatisticas[0].total_pedidos_0;
        total_ped_1 = shared_mem_estatisticas[0].total_pedidos_1;
        cont_0 = shared_mem_estatisticas[0].conta_0;
        cont_1 = shared_mem_estatisticas[0].conta_1;
        ...
        shared_mem_estatisticas[0].media_0 = med_0;
        shared_mem_estatisticas[0].media_1 = med_1;
		shared_mem_estatisticas[0].total_pedidos_0 = total_ped_0;
        shared_mem_estatisticas[0].total_pedidos_1 = total_ped_1;
        shared_mem_estatisticas[0].conta_0 = cont_0;
        shared_mem_estatisticas[0].conta_1 = cont_1;
	    ...
	    //escreve para MMF
		if (conta_global < 100) {
            strcpy(file->nome_ficheiro,teste0);
            file->tipo_pedido = teste1;
            file->hora_pedido = teste2;
            file->hora_servido = teste3;
            conta_global++;
            //memcpy(&shared_mem[buffer_mem_read], fd , sizeof(Stati));

        } else {
            //desmapeia
            munmap(file, statbuf.st_size);

            //aumenta ficheiro + 100
            lseek(fd, sizeof(Stati)*100 , SEEK_SET);
            write(fd,"", 1);
            conta_global = 0;
            //mapeia ficheiro

            //escreve
            strcpy(teste0, file[0].nome_ficheiro);
            file->tipo_pedido = teste1;
            file->hora_pedido = teste2;
            file->hora_servido = teste3;
            conta_global++;
...
void *listening_pipe(void *arg) {
	#if DEBUG
		printf("Thread à escuta do PIPE ...\n");
	#endif
